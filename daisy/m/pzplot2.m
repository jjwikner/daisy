function ph = pzplot(H, color, markersize, list)

% function pzplot2(H,color='b',markersize=5,list=0)
% Plot the poles and zeros of a transfer function.
% If list is non-zero, a list of the poles and zeros 
...
ph(1) = plot(real(p), imag(p), pole_fmt);
set(ph(1), 'markersize', markersize);

if ~isempty(z)
    hold_status = ishold;
    if ~hold_status; hold on; end;
    ph(end+1) = plot(real(z), imag(z), zero_fmt);
    set(ph(end),'markersize',markersize);
...
hold_status = ishold;
hold on
ph(end+1) = plot(exp(j*2*pi*(0:0.01:1)),'-.');
axis('equal');
limits = axis;
ph(end+1:(end+1+1)) = plot([0 0],limits(3:4),'k:',limits(1:2),[0 0],'k:');
...
ph(end+1) = ylabel('Imaginary');
ph(end+1) = xlabel('Real');
ph(end+1) = title('Pole-Zero Diagram');
