...
holidays([1 6 7 13 14 20 21 27 28]) = 1;
deltay   = 0.008;
ymargin  = 0.1;
xwdth    = 0.08;
xmargin  = 0.02;
...
ym = ymargin;
xs = 1.1;
yhgt     = [(1-2*ymargin) / (no_days-1) - 0.5*deltay];
...
lstflexh = ...
    putFld([xm+8*xwdth*xs 1-3*ym+yhgt xwdth yhgt], ...
...
for m = 1:no_days
  ypos = 1-(3*ym + (m-1)*(yhgt+deltay/2));
  turban = line([0 1],[1 1]*(ypos));
  set(turban,'LineStyle',':');

  daynmh(m) = ...
      putTxt([xm ypos xwdth yhgt], day{mod(m+dayshift-1,7)+1}, hmnthrpt);
...
%putFrm([0.05 0.01 0.9 0.07],'Controls and statistics',hmnthrpt);
putBtn([0.1 0.5*ym 0.15 yhgt],'Save data', 'micrpt(''save'')');
putBtn([0.1 0.5*ym+yhgt*1.1 0.15 yhgt],'Load data', 'micrpt(''load'')');

putBtn([0.7 0.5*ym+yhgt*1.1 0.15 yhgt],'Close', 'close(gcf)');
putBtn([0.7 0.5*ym  0.15 yhgt],'Print', 'printdlg(gcf)');

putTxt([0.1+0.2 0.5*ym+yhgt*1.1 0.2 yhgt],'Total worktime:', hmnthrpt);
totwrkh = putTxt([0.1+0.2+0.2 0.5*ym+yhgt*1.1 0.1 yhgt], ...
    'djdjd', hmnthrpt);

putTxt([0.1+0.2 0.5*ym 0.2 yhgt],'Total flextime:', hmnthrpt);
totackh = putTxt([0.1+0.2+0.2 0.5*ym 0.1 yhgt], ...
    'djdjd', hmnthrpt);
...
elseif (strcmp('update',state))
  totwrk = 0;
  for m = 1:no_days
    if ((tme2num(get(dstart1h(m),'String')))<0) 
      set(dstart1h(m), 'String', 0);
    elseif  ((tme2num(get(dstart1h(m),'String')))>24) 
      set(dstart1h(m), 'String', 24);
    end;
    if ((tme2num(get(dstart2h(m),'String')))<0) 
      set(dstart2h(m), 'String', 0);
    elseif  ((tme2num(get(dstart2h(m),'String')))>24) 
      set(dstart2h(m), 'String', 24);
    end;
    if ((tme2num(get(dend1h(m),'String')))<0) 
      set(dend1h(m), 'String', 0);
    elseif  ((tme2num(get(dend1h(m),'String')))>24) 
      set(dend1h(m), 'String', 24);
    end;
    if ((tme2num(get(dend1h(m),'String')))<(tme2num(get(dstart1h(m),'String')))) 
      set(dend1h(m), 'String', get(dstart1h(m),'String'));
    end;
    if ((tme2num(get(dend2h(m),'String')))<0) 
      set(dend2h(m), 'String', 0);
    elseif  ((tme2num(get(dend2h(m),'String')))>24) 
      set(dend2h(m), 'String', 24);
    end;
    if get(dlunchh(m),'Value')
