...
global WITNESSLINE YVERTICAL DATAFILE funcnames;
...
funcnames = ['sine' 'ramp' 'sawt' 'tria' 'sqR0' 'sq+-'];
AMPMAX = 0.0004;
AMPMIN = -0.0004;
NOB = 8;
FUNCTYPE = 1;
DIGITALVALUES = AD_convert(funcnames(4*(FUNCTYPE-1)+1:4*FUNCTYPE), ...
    AMPMAX,2,128,NOB,1,AMPMAX/(2^(NOB-1)-1));
STARTS = 1;
...
PLOTTYPE = 1;
FUNCTYPE = 1;
YVALUES = DA_convert(DIGITALVALUES(STARTS:STOPS,1:NOB), ...
    1,AMPMAX/(2^(NOB-1)-1),1);
...
elseif strcmp(ACT,'setdata')
	...
	YVALUES = DNL(DIGITALVALUES(STARTS:STOPS,1:NOB),funcnames(4*(FUNCTYPE-1)+1:4*FUNCTYPE),AMPMAX, AMPMIN);
	XVALUES = (-(size(YVALUES,2)-1)/2:(size(YVALUES,2)-1)/2);
	XFACTOR = 1;
	XOFFSET = (size(YVALUES,2)+1)/2;
	...
	YVALUES = INL(DIGITALVALUES(STARTS:STOPS,1:NOB),funcnames(4*(FUNCTYPE-1)+1:4*FUNCTYPE),AMPMAX, AMPMIN);
	XVALUES = (-(size(YVALUES,2)-1)/2:(size(YVALUES,2)-1)/2);
	XFACTOR = 1;
	XOFFSET = (size(YVALUES,2)+1)/2;
