function [SNDR, carr_pos] = ... 
     estSNDR(input_signal, os_ratio, exp_carr_pos);

%
% function [SNDR, carr_pos] = ... 
%     estSNDR(input_signal, os_ratio, exp_carr_pos);
%
% The SNDR is estimated from the input_signal. Using the
...
% 
% estSNDR(input_signal) finds the strongest ac signal component
...
% Changes to the code at Linköping university, 001017, JJW
% correction so that we do not get any negative indeces.

k = length(input_signal);
m = floor(k/2);
spectrum = spect20(input_signal,'blackman');
...
deltac = 0.02 ;
carr_interval = ...
    max(1, carr_pos - floor(deltac*m)) : ...
    min(m, carr_pos + floor(deltac*m));
dc_interval = 1:max(1,floor(deltac*m/3));
